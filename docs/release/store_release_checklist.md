...
- Первое сопряжение в v1 требует интернет: обмен ключами реализован только
  через signaling Worker. BLE-пейринг (обмен ключами по GATT без сети,
  спека §4 «работа без интернета») — roadmap-фича: все примитивы уже в
  коде (сканер/advertise/GATT-клиент/кодек в transport/ble, ECDH+SAS в
  crypto), не хватает только хендшейк-флоу поверх BLE. После сопряжения
  BLE и локальная сеть работают полностью офлайн уже сейчас.
