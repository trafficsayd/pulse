...
- Кастомные звуки Sneak In отложены до v1.1: в v1 сигнал — вибрация +
  эмодзи-вспышка (аудио-пайплайна в коде нет, .opus-файлы не шипуются;
  генератор заглушек лежит в tool/). UI и листинги сформулированы без
  обещания звука; фоновый режим `audio` удалён из Info.plist как
  необоснованный (реджект-риск 2.5.4) — вернуть вместе с реальным аудио.
- Расхождение со спекой §7/§8 (звук сигнала у получателя) — осознанное
  ограничение v1; спека остаётся источником истины для v1.1.
- Сопряжение работает и без интернета: при недоступном signaling Worker
  контроллер автоматически падает на BLE-рандеву (хост advertise'ит
  GATT-сервис, joiner подключается центральной ролью; обмен публичными
  ключами кадрами `pair_hello`/`pair_reply`, SAS-подтверждение — как
  онлайн). Ограничение радио: advertise умеет только Android, поэтому
  офлайн-пейрингу нужно, чтобы хотя бы «Создать пару» нажали на Android.
  Пара iPhone+iPhone целиком офлайн — v-next (нужен CBPeripheralManager).
  После сопряжения BLE и локальная сеть офлайн на всех платформах.
